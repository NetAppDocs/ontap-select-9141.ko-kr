...
[NOTE]
====
ONTAP Select 배포 관리 유틸리티를 다운로드하려면 * 등록된 NetApp 지원 사이트 계정이 필요합니다. 계정이 없는 경우 을 참조하십시오 https://mysupport.netapp.com/site/user/registration["사용자 등록"^]. * https://mysupport.netapp.com/site/downloads/evaluation/ontap-select["최종 사용자 사용권 계약에 동의합니다"^]평가판 라이센스가 있는 ONTAP Select 배포용.
